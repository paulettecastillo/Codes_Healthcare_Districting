...
def main(modelo,var,symmetry):
    ...
    if symmetry==1:
        print("entro a el modelo DSF con symmetry breaking")
...
def root_node_model(model, where):
        ...
        if where == GRB.Callback.MIPNODE:
            if model.cbGet(GRB.Callback.MIPNODE_NODCNT) == 0 and count_1==0:
                ...
                obj_bound_root_node = model.cbGet(GRB.Callback.MIPNODE_OBJBND)  # Límite inferior
                obj_value_root_node = model.cbGet(GRB.Callback.MIPNODE_OBJBST)  # Mejor solución hasta ahora
...
def archivos(m,nombre_modelo,start_time,obj_bound_root_node,obj_value_root_node,best_Sol,best_nodes,best_time): 
    ...
    file_performance = f"Performance_{nombre_modelo}.txt"
    #------------------------------------------------------- MODEL INFEASIBLE-------------------------------------------------------------------------------------------------------                     
...
#----------------------------------------------------------PRINTS SOLUCTION---------------------------------------------------------------------------------------------------------------------------------
        ...
        if nombre_modelo=='MTZ'or nombre_modelo=='MTZ_LR' or nombre_modelo=='MTZ_S':
        ...
        if nombre_modelo=='MCF_I' or nombre_modelo=='MCF_LR':
            ...
            lista_f={}
            for (i,k,q,l) in m._f.keys():
                lista_f[i,k,q,l]= m._f[i,k,q,l].x
            ...
            Solution[dis, beta] = [(i, j, k) for k in V for (i,j) in EF if m._z[i, j, k].x > 0.1]
        ...
        if nombre_modelo=='SCFF1' or nombre_modelo=='SCFF1_LR' or nombre_modelo=='SCFF1_S':
            ...
            for (i,j,k) in m._f.keys():
                if m._f[i,j,k].x >0.0003:
                    print('f', (i,j,k), m._f[i,j,k].x)
        ...
        if nombre_modelo=='SCFF2' or nombre_modelo=='SCFF2_LR' or nombre_modelo=='SCFF2_S':
        ...
        if nombre_modelo=='SCFF3' or nombre_modelo=='SCFF3_LR' or nombre_modelo=='SCFF3_S':
        ...
        if nombre_modelo=='SHIR' or nombre_modelo=='SHIR_LR':
...
def archivos1(m,nombre_modelo,start_time_1,obj_bound_root_node,obj_value_root_node,best_Sol,best_nodes,best_time): 
    ...
    #--------------------------------------------------------FEASIBLE------------------------------------------------------------------------------------------------------------------------------
        ...
        if nombre_modelo=='MTZ_LR'or nombre_modelo=='MCFF_LR' or nombre_modelo=='SCFF1_LR' or nombre_modelo=='SCFF2_LR':
            gap_opt='-'              
...
#rescatamos el clique mas grande que tenemos
clique=[]
...
# IMPORTANTE: Recargar el módulo models para que use los valores actualizados
import models
...
print(nodos_finales, 'nodos antes infactibilidad')
