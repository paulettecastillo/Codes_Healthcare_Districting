...
def max_clique():
    ...
    #m.addConstr(gp.quicksum(m._x[i] for i in V)<=len(K))
...
def model_a_z(var):
    ...
    #3 all of nodes must to have one father
    for j in V:
...
def MCF(m):
    ...
    #5 Cada comuna tiene como entrada una sola Comuna
    for k in V:
